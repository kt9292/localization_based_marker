...
class Aruco_Res():
    ...
    result = cv2.VideoWriter('aruco_tracking.mp4',cv2.VideoWriter_fourcc(*'MP4V'),10, size)
    ...
    def ArucoPositioning(self):
        ...
        if ids is not None:
            ...
            for i in range(rvecs.shape[0]):
                cv2.drawFrameAxes(frame, mtx, dist, rvecs[i, :, :], tvecs[i, :, :], 0.03)
